...
# in bulk. (This version works for files from RCSB, with disulfide bonds
# from CHARMM).
...
do
	...
	then
		...
		do
			...
			obabel disulfide_bonds.pdb -h --partialcharge $charge -xrh \
				-O  receptor_${charge}_disulfide.pdbqt &
		done
	fi
done
