...
do
	rm -rf $top/GROMACS/water_model_effect/gro_correction_and_charges_em/$dir
	mkdir -p $top/GROMACS/water_model_effect/gro_correction_and_charges_em/$dir
	cp $top/GROMACS/water_model_effect/{,gro_correction_and_charges_em/}$dir/em.gro
	cp $top/GROMACS/water_model_effect/{,gro_correction_and_charges_em/}$dir/$(echo $dir | \
		tr '[a-z]' '[A-Z]')_original.pdb
	cd $top/GROMACS/water_model_effect/gro_correction_and_charges_em/$dir
	cat em.gro | grep -v 'CLA' | \
		grep -v 'CL ' | \
		grep -v 'NA ' | \
		grep -v 'SOL' > em_wrong_no.gro
	n_of_lines=$(cat em_wrong_no.gro | \
		tail -n 2 | head -n 1 | \
		sed -e "s/\ \+/*/g" | \
		cut -d '*' -f 4)
	cat em_wrong_no.gro | \
