...
do
	rm -rf $top/GROMACS/water_model_effect/gro_correction_and_charges_npt/$dir
	mkdir -p $top/GROMACS/water_model_effect/gro_correction_and_charges_npt/$dir
	cp $top/GROMACS/water_model_effect/{,gro_correction_and_charges_npt/}$dir/npt.gro
	cp $top/GROMACS/water_model_effect/{,gro_correction_and_charges_npt/}$dir/$(echo $dir | \
		tr '[a-z]' '[A-Z]')_original.pdb
	cd $top/GROMACS/water_model_effect/gro_correction_and_charges_npt/$dir
	cat npt.gro | grep -v 'CLA' | \
		grep -v 'CL ' | \
		grep -v 'NA ' | \
		grep -v 'SOL' > npt_wrong_no.gro
	n_of_lines=$(cat npt_wrong_no.gro | \
		tail -n 2 | head -n 1 | \
		sed -e "s/\ \+/*/g" | \
		cut -d '*' -f 4)
	cat npt_wrong_no.gro | \
