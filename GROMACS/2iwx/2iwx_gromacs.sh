...
gmx pdb2gmx -f 2IWX_original.pdb \
		-o 2IWX_forcefield.gro \
		-water spc \
		-ff gromos53a6_hsd \
		-ignh
...
gmx editconf -f 2IWX_forcefield.gro -o 2IWX_box.gro -c -d 1.0 -bt cubic

# Solvating box created previously.
gmx solvate -cp 2IWX_box.gro -cs spc216.gro -o 2IWX_solvated.gro -p topol.top

# Including ions to obtain a neutral charge
gmx grompp -f ions.mdp -c 2IWX_solvated.gro -p topol.top -o ions.tpr -maxwarn 1
# Substitute only solvent molecules.
echo "SOL" | \
	gmx genion -s ions.tpr -o 2IWX_ions.gro \
		-p topol.top -pname NA -nname CL -neutral

# And we start the energy minimization.
gmx grompp -f minim.mdp -c 2IWX_ions.gro -p topol.top -o em.tpr -maxwarn 1
