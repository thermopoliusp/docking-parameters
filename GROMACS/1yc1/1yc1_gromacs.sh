...
# of human Hsp90-alpha using GROMACS.
...
gmx pdb2gmx -f 1YC1_original.pdb \
		-o 1YC1_forcefield.gro \
		-water spc \
		-ff gromos53a6_hsd \
		-ignh
...
gmx editconf -f 1YC1_forcefield.gro -o 1YC1_box.gro -c -d 1.0 -bt cubic

# Solvating box created previously.
gmx solvate -cp 1YC1_box.gro -cs spc216.gro -o 1YC1_solvated.gro -p topol.top

# Including ions to obtain a neutral charge
gmx grompp -f ions.mdp -c 1YC1_solvated.gro -p topol.top -o ions.tpr -maxwarn 1
# Substitute only solvent molecules.
echo "SOL" | \
	gmx genion -s ions.tpr -o 1YC1_ions.gro \
		-p topol.top -pname NA -nname CL -neutral

# And we start the energy minimization.
gmx grompp -f minim.mdp -c 1YC1_ions.gro -p topol.top -o em.tpr -maxwarn 1
