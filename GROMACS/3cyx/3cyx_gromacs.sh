...
gmx pdb2gmx -f 3CYX_original.pdb \
		-o 3CYX_forcefield.gro \
		-water tip3p \
		-ff charmm36 \
		-ignh
...
gmx editconf -f 3CYX_forcefield.gro -o 3CYX_box.gro -c -d 1.0 -bt cubic

# Solvating box created previously.
gmx solvate -cp 3CYX_box.gro -cs spc216.gro -o 3CYX_solvated.gro -p topol.top

# Including ions to obtain a neutral charge
gmx grompp -f ions.mdp -c 3CYX_solvated.gro -p topol.top -o ions.tpr
# Substitute only solvent molecules.
echo "SOL" | \
	gmx genion -s ions.tpr -o 3CYX_ions.gro \
...
sed -i 's/\ CL/\CLA/g' 3CYX_ions.gro
sed -i 's/CL/CLA/g' topol.top

# And we start the energy minimization.
gmx grompp -f minim.mdp -c 3CYX_ions.gro -p topol.top -o em.tpr
