...
gmx pdb2gmx -f 3OV1_original.pdb \
		-o 3OV1_forcefield.gro \
		-water tip3p \
		-ff charmm27 \
		-ignh
...
gmx editconf -f 3OV1_forcefield.gro -o 3OV1_box.gro -c -d 1.0 -bt cubic

# Solvating box created previously.
gmx solvate -cp 3OV1_box.gro -cs spc216.gro -o 3OV1_solvated.gro -p topol.top

# Including ions to obtain a neutral charge
gmx grompp -f ions.mdp -c 3OV1_solvated.gro -p topol.top -o ions.tpr
# Substitute only solvent molecules.
echo "SOL" | \
	gmx genion -s ions.tpr -o 3OV1_ions.gro \
		-p topol.top -pname NA -nname CL -neutral

# And we start the energy minimization.
gmx grompp -f minim.mdp -c 3OV1_ions.gro -p topol.top -o em.tpr
