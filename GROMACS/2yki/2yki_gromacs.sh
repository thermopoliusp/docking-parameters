...
gmx pdb2gmx -f 2YKI_original.pdb \
		-o 2YKI_forcefield.gro \
		-water spc \
		-ff gromos53a6_hsd \
		-ignh
...
gmx editconf -f 2YKI_forcefield.gro -o 2YKI_box.gro -c -d 1.0 -bt cubic

# Solvating box created previously.
gmx solvate -cp 2YKI_box.gro -cs spc216.gro -o 2YKI_solvated.gro -p topol.top

# Including ions to obtain a neutral charge
gmx grompp -f ions.mdp -c 2YKI_solvated.gro -p topol.top -o ions.tpr -maxwarn 1
# Substitute only solvent molecules.
echo "SOL" | \
	gmx genion -s ions.tpr -o 2YKI_ions.gro \
		-p topol.top -pname NA -nname CL -neutral

# And we start the energy minimization.
gmx grompp -f minim.mdp -c 2YKI_ions.gro -p topol.top -o em.tpr -maxwarn 1
